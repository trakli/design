...
const targets = computed(() => props.budget?.targets ?? []);
const categories = computed(() => targets.value.filter((tr) => tr.type === 'category'));
const groups = computed(() => targets.value.filter((tr) => tr.type === 'group'));
const wallets = computed(() => targets.value.filter((tr) => tr.type === 'wallet'));
const hasAnyTargets = computed(() => targets.value.length > 0);

const summaryFor = (type, items) => {
...
const effectiveLimit = computed(() =>
  Number(progress.value?.effective_limit ?? Number(props.budget?.amount ?? 0))
);
const remaining = computed(() => Number(progress.value?.remaining ?? (effectiveLimit.value - netSpent.value)));
...
const formatMoney = (value) => {
  return formatAmount(`${value} ${props.budget?.currency || 'USD'}`);
};
