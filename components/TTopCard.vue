  ...
  <div class="entity-header surface surface--brand">
    ...
    >
      <defs>
        <radialGradient id="topcard-bloom" cx="92%" cy="50%" r="50%">
          <stop offset="0%" stop-color="var(--surface-accent, var(--color-primary-light))" stop-opacity="0.45" />
          <stop offset="100%" stop-color="var(--surface-accent, var(--color-primary-light))" stop-opacity="0" />
        </radialGradient>
      </defs>
      ...
      <circle cx="980" cy="98" r="6" fill="var(--surface-accent, var(--color-primary))" opacity="0.8" />
    </svg>
    <div class="header-content">
        ...
        <div class="title-row">
          ...
          <div class="title-text">
            <span class="breadcrumb-current">
              <span class="breadcrumb-home" @click="handleHomeClick">{{ homeLabel || t('Home') }}</span>
      ...
      <div class="action-buttons">
        <slot name="info">
          <TInfoButton v-if="showInfoButton" />
        </slot>
        <slot name="actions" />
        <TButton
          v-if="showAddButton"
...
const emit = defineEmits(['add', 'home-click']);

const i18n = typeof useI18n === 'function' ? useI18n() : null;
const t = i18n?.t || ((key, params) => {
  if (params?.item) return `Add ${params.item}`;
  return key;
});
...
const handleHomeClick = () => {
  emit('home-click', props.homeTo);
  if (typeof useRouter === 'function') {
    try {
      const router = useRouter();
      router?.push?.(props.homeTo);
    } catch {
      // safe fallback if useRouter not available
    }
  }
};
</script>
