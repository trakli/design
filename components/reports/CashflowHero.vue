  ...
  <section class="hero">
    ...
    <div class="sats">
      <article class="sat">
        ...
        <div class="sat-spark">
          <SparkLine :values="incomeTrend" color="var(--color-income, #10b981)" :height="22" />
        </div>
      </article>
      <article class="sat">
        <header class="sat-head">
        ...
        <div class="sat-spark">
          <SparkLine :values="expenseTrend" color="var(--color-expense, #ef4444)" :height="22" />
        </div>
      </article>
      <article class="sat">
        <header class="sat-head">
        ...
        <div class="sat-value-row">
          <p class="sat-value">{{ ((totals.savingsRate || 0) * 100).toFixed(0) }}%</p>
          <span
            v-if="compareEnabled"
            class="delta"
        ...
        <p v-if="compareEnabled" class="whisper-line">
          {{ t('vs') }} {{ ((totals.prevSavingsRate || 0) * 100).toFixed(0) }}%
        </p>
        <div class="sat-spark">
          <SparkLine :values="rateTrend" color="var(--color-text-secondary, #6b7280)" :height="22" />
        </div>
      </article>
    </div>
  </section>
...
const format = (n) => {
  if (props.formatter) return props.formatter(n, props.currency);
  return formatShortAmount(`${n ?? 0} ${props.currency}`);
};

const incomeTrend = computed(() => props.trailing.map((m) => m.income ?? 0));
const expenseTrend = computed(() => props.trailing.map((m) => m.expense ?? 0));
const netTrend = computed(() => props.trailing.map((m) => m.net ?? 0));
const rateTrend = computed(() =>
  props.trailing.map((m) => {
