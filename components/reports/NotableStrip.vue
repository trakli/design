  ...
  <section class="strip">
    <article class="note-card note-card--biggest">
      ...
      <template v-if="notable?.biggestExpense">
        <p class="note-value">
          {{ formatValue(notable.biggestExpense.amount) }}
        </p>
        <p class="note-meta">
    ...
    <article class="note-card note-card--swing">
      ...
      <template v-if="notable?.biggestCategorySwing">
        ...
        <p class="note-meta">
          {{ formatValue(notable.biggestCategorySwing.fromAmount) }}
          <ArrowRight :size="12" class="inline-ico" />
          {{ formatValue(notable.biggestCategorySwing.toAmount) }}
        </p>
        <p
          class="note-pill"
    ...
    <article class="note-card note-card--first">
      ...
      <p class="note-value">{{ notable?.firstTimePayees?.count ?? 0 }}</p>
    ...
    <article class="note-card note-card--streak">
      ...
      <p class="note-value">
        {{ notable?.longestNoSpendStreak?.days ?? 0 }}
...
const formatValue = (val) => {
  if (props.formatter) return props.formatter(val, props.currency);
  return formatShortAmount(`${val ?? 0} ${props.currency}`);
};
...
const formatDate = (iso) => {
  ...
  try {
    return new Intl.DateTimeFormat(locale.value || 'en', { month: 'short', day: 'numeric' }).format(d);
  } catch {
    return iso;
  }
};
